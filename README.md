...
- **Session lineage** — A session that continued another's work reads as one. It carries a *"↳ continued from …"* line, and its idle ancestors fold under it (a *"N earlier in thread"* toggle) instead of cluttering the sidebar as separate rows. Hard links (a fork, a Hermes parent) are shown as fact; a Claude `/clear` — which records no back-link — is inferred from the moment the old session stopped writing and the new one began, and is labelled a **guess**. That same inference also fixes the case where a `/clear` with several sessions live in one project left the source stuck reading as *running* and the tab attached to the dead id. See [spec 13](docs/specs/13-session-lineage.md).
