...
- **Session lineage** — A session that continued another's work reads as one. It carries a *"↳ continued from …"* line, and its idle ancestors fold under it (a *"N earlier in thread"* toggle) instead of cluttering the sidebar as separate rows. Hard links (a fork, a Hermes parent) are shown as fact; a Claude `/clear` — which records no back-link — is inferred and labelled a **guess**. A `/clear` also folds the old session's row onto the new one so the tab follows and the source stops reading as *running* — done safely only when a single session is live in the folder (with several live it stays as-is rather than risk mis-attributing the clear). See [spec 13](docs/specs/13-session-lineage.md).
