...
// The tag becomes part of a file name, and `removeSubagentExtension` deletes a file by that name. A plain
// token is all it ever needs (it is `crypto.randomUUID()` at the one call site) and all it will accept.
const SAFE_TAG = /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/;
const FILE_PREFIX = 'pi-subagent-';
...
// A lean subagent tool in the shape of Pi's own example extension (examples/extensions/subagent, MIT).
...
/**
 * Write the extension for one spawn, or decline. Answers `{ args, cleanup }` or null — null for anything
 * it cannot or will not do, because a launch must never fail over this.
 *
 * A raw `fs.writeFileSync` is right here for the reason `prompt-templates.js` gives: the file is made
 * fresh for this spawn under the app's own data directory, and nothing else writes it.
 */
function writeSubagentExtension({ dir, tag, options, log } = {}) {
  if (!dir || !tag || !SAFE_TAG.test(String(tag))) return null;
  // Strictly `true`: the field defaults OFF, and "nobody said anything" has to mean no.
  if (!options || options[OPTION_ID] !== true) return null;
  const file = path.join(dir, `${FILE_PREFIX}${tag}.ts`);
  try {
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(file, extensionSource({ agentsDir: agentsDirFrom(options) }), 'utf8');
  } catch (err) {
    if (log && typeof log.warn === 'function') log.warn(`[pi-subagent] could not write the extension: ${err.message}`);
    return null;
  }
  if (log && typeof log.debug === 'function') log.debug(`[pi-subagent] extension written for one spawn`);
  return { args: ['--extension', file], cleanup: file };
}

/** Remove a file this module wrote, and nothing else. */
function removeSubagentExtension(file, log) {
  if (!file || !OWN_FILE.test(path.basename(String(file)))) return;
  try { fs.rmSync(file, { force: true }); }
  catch (err) { if (log && typeof log.warn === 'function') log.warn(`[pi-subagent] cleanup failed: ${err.message}`); }
}
