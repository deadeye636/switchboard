...
//   * REGISTERS Pi's own session commands (`./session-commands.js`, #642/#643): `/login`, `/logout`,
//     `/model`, `/thinking`, `/compact`, and a line of explanation for the rest of Pi's terminal commands,
//     which over RPC would otherwise reach the model as a prompt.
//
...
// Pi's own session commands — `/login`, `/model`, `/compact` and the rest (#642, #643). A section of this file
// rather than of the resources extension, because the terminal backend has them natively; its header says why
// they are registered at load. Registered with the gate on or off: they are commands the user types, not tools
// the agent calls.
const { commandsSource } = require('./session-commands');
...
function extensionSource({ gate = true } = {}) {
  ...
  return `// Generated by Switchboard for one Pi session. Safe to delete.\n`
    ...
    + marker
    + approvals
    // A Pi whose API cannot take them keeps the marker and the gate: the commands are the part to lose.
    + `  try { if (typeof pi.registerCommand === "function") registerSessionCommands(pi); } catch {}\n`
    + `}\n`;
}
