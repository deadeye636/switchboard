...
// app.js and consumes its state one-way — main reaches the listeners over IPC, app.js never calls back,
// so nothing here needs a guard.
...
// The tag-filter chip bar and the archive/star/running/today/favorite toggles are
// shell/sidebar-filters.js (#218/#228). It loads AFTER app.js and writes the filter state that lives
// here (projectTagMap/sessionTagMap/activeProjectTagFilter/activeSessionTagFilter and the show* flags,
// all read by the sidebar render above). app.js calls back into it guarded (see that file).
...
// The sidebar search bar — debounce, 3-char floor, per-tab dispatch, Enter/refresh reindex — is
// shell/search-bar.js (#218/#228). It loads AFTER app.js (it reads searchInput/searchBar at parse
// time) and writes searchMatchIds/searchMatchProjectPaths, which stay declared here for the sidebar
// render to read.
...
// Session timeline viewer (renderTimelineViewer, renderTimelineFilters, showTimelineViewer + the two filter
// listeners) → session/timeline-view.js (#302). Loaded after app.js, since those listeners read app.js DOM
// consts at parse time.
    ...
    window.api.onConfirmClose(async (warning) => {
      ...
      try {
        ok = await showControlDialog({
          ...
          // Main words this; the literal is the fallback for a warning that carries no label.
...
loadProjects().then(async () => {
  // Build the project tag-filter chip bar once projects are loaded (#98). _refreshProjectTagFilter is in
  // shell/sidebar-filters.js (loads after app.js, #228); `?.` so this boot callback degrades to a no-op if
  // it runs before that script parses, like the sibling call at the top of reapplyGlobalSettings.
  window._refreshProjectTagFilter?.();
