      ...
      if (backend.providesSessionResources === true && typeof backend.buildSessionResources === 'function') {
        try {
          ...
          if (built && built.source) {
            ctx.log.info(`[session-resources] session=${sessionId} from ${built.source}: ${(built.skills || []).length} skill dir(s), ${(built.commands || []).length} command dir(s), ${(built.agents || []).length} agent dir(s), ${(built.mcpServers || []).length} MCP server(s), ${(built.hooks || []).length} hook(s), ${(built.dropped || []).length} dropped`);
