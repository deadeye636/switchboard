...
| `resolveLineage(row)` | → `{lineageParentId, lineageKind}` \| `null` — which session this one continued (Spec 13). Claude: a fork's `forkedFrom`; Hermes: `parent_session_id`; Pi: a fork's `parentSession` path (only a forked session has one); Codex and agy `null`. The core stamps it at one sink and never reads a backend's format (#193/#223). |
