...
| `configFields` | this CLI's launch options. **The Settings page and the Configure dialog are generated from it.** A field may declare `appliesAt: 'spawn'` (applied by `app/terminal/spawn.js`, not part of the argv) or `requires: '<other>'` (meaningless on its own). A **select** field may also declare `retiredChoices: { <dead>: <surviving> }` (#617) — a value its CLI has stopped accepting, and what a setting saved under it becomes. The core rewrites the stored value once, in every store that can hold one (`app/settings.js`); the mapping itself is the backend's, because it is one CLI's vocabulary. Options that belong to **Switchboard** rather than to a CLI — today `preLaunchCmd` — are added to every backend by the registry (`UNIVERSAL_FIELDS`), not copied into each descriptor. |
...
| `transport` / `transcriptsOf` / `rpc` | OPTIONAL, for a backend DRIVEN over a runtime protocol instead of a PTY (#568, spec 30). `transport` names it (`'rpc'`) and makes the spawn path start a child on a pipe (`src/app/agent-rpc.js`); `transcriptsOf` names the backend whose store and rows it uses, because two backends cannot own one row; `rpc` is the half that turns the CLI's lines into the app's own ops and back. `pi-native` is the one user. |
| `providesPromptTemplates` + `buildPromptTemplates` / `releasePromptTemplates` | OPTIONAL. The app's own `/handoff` and `/plan` handed to the CLI as prompt templates for one spawn (#569, spec 29). Pi declares it, and `pi-native` forwards it. |
| `providesRuntimeExtension` + `buildRuntimeExtension` / `releaseRuntimeExtension` | OPTIONAL. The per-spawn extension a runtime-driven backend starts with: the transport marker and the approval gate (#568, spec 30). |
| `providesSessionResources` + `buildSessionResources` / `releaseSessionResources` | OPTIONAL. Everything a session is GIVEN beyond its own setup, in ONE per-spawn extension whose parts are sections: the subagent tool (#634), another CLI's commands (#632) and agents (#639). New per-spawn features attach as a section here, not as a new hook pair. |
| `sharedResources` / `acceptsSharedResources` / `trustsProjectResources` / `declinesSharedResource` | "Resources from" (#632, #639, spec 31). A SOURCE declares which of its `listResources` rows may leave it and, as data, a `commandDialect` and an `agentDialect`; a TARGET declares the kinds it takes, whether this launch may be handed a project's directories, and optionally a kind it declines for one launch. `src/app/resource-sources.js` is the one resolver. An agent's tools meet in `src/backends/tool-vocabulary.js`. |
| `noteTurnQueue` | the other half of `readTurnQueue` above, for a backend that is TOLD a turn is queued instead of reading it (Pi's binding extension, #530); a template that forwards the one must forward the other (#605). |
| `changelogSource` | where the CLI publishes what changed, read by `npm run backends:changelog-check` (#528); `null` for a CLI with no public changelog. |
| field-level `appliedBy` / `choicesFrom` | on a `configFields` entry, not on the descriptor. `appliedBy` names the spawn hook that applies an option the argv never carries (#569); `choicesFrom: 'sharedResourceSources'` lets the core fill a select with other backends, which the backend's own folder cannot name (#632). |
...
on every backend — `plansDir`, `memorySources`, `resolveLineage`, `openedWithCommand`, `contextWindow`, `cliHomeEnv`, `transcriptPathFor` and
`listResources` are declared by all five — and several exist *in order to decline*: agy's `cliHomeEnv`
returns null, agy's `resolveLineage` returns null, Hermes' `plansDir` returns null. Presence says a backend
...
**And a declared answer goes stale where a derived one could not.** Codex' row said
`lineage: { state: 'no' }` for as long as its `resolveLineage` returned null — then #229 wired the hook and
the row still said `no`, because nothing derives it and the guard only refuses a `yes` whose field is
absent, never a `no` whose field has arrived. That is the price of declaring, and it is still the right
trade: the cure is to change the row in the same commit as the hook, not to start deriving.
