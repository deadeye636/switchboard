...
| `pageKeyTarget` | `'pty'` when bare PageUp/PageDown are application keys the backend TUI handles (Claude), `'viewport'` when its TUI ignores them and the session history lives in xterm's scrollback (Codex, Pi). The renderer never branches on a backend id — and the per-backend answer is **measured in a live session and pinned** in `test/terminal-page-scroll.test.js`, because a change aimed at one backend twice moved backends that were not in its scope. |
