...
| E5 | **No trust, no start (#655).** A session starts only in a project Pi trusts. Without a saved yes the spawn is refused before anything runs, and the app asks the trust question itself — the same confirm the Projects manager uses — and starts again on a yes. Only the saved answer counts: the per-run "Project trust for this run" option is not offered here, and a stored value is ignored. | Over RPC Pi asks nobody whether a project may load its own settings and extensions, and a start with the project half left out is a session with less than the user expects and nothing saying so. The same gate applies to the Claude backend driven over a pipe (#653), where the CLI skips its trust dialog outright — one implementation, switched on by `trustBeforeStart` on the descriptor. A launch nobody made at the keyboard (the restore at start, the boot fallback) asks nothing and leaves the refusal in the tab; the trigger watcher only writes into running sessions and cannot hit it. |
