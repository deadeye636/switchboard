...
| 23 | [Command palette](23-command-palette.md) — **built**; why it is a fourth description on the shared picker core rather than a second popover, the three things that core lacked for a palette with no terminal, why the ranking is its own module and not the sidebar's search, and the actions that register themselves | #274 | Yes — the picker core, one ranking module and the owners that declare an action |
