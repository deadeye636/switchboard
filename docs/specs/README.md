...
| 31 | [Resources from: a Pi session takes over another CLI's skills and commands](31-resources-from.md) — **built**; why one source and not several, why a project's directories need Pi's trust, why commands go through an extension instead of Pi's templates and what their shell lines may run, the seam that names no backend in the core, what the settings preview shows, and what does not come along (hooks, MCP, agents) | #632 | No — declarations on the backends, a core module, one per-spawn extension, a settings control |
