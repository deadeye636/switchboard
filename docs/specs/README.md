...
| 21 | [Skills](21-skills.md) — **built**; the picker that hands a skill to the CLI in the terminal, the measured answer for each backend on whether it can run one from its prompt, the app's own skills, and the palette core every picker now shares | #462 | Yes — one main module, the descriptors, and the terminal pickers |
...
| 25 | [Handoffs live in the project](25-handoffs.md) — **built**; why a packet had to become a file before it could be handed to a running session, the export that runs before the table is dropped and the row that stops it, why a read list and a write target are two settings, and the failure a database insert never had | #468, #469 | Yes — one main module, the renderer flow and a fifth picker description |
| 26 | [The settings screen](26-settings-screen.md) — **built**; why a category is a subject and the counts are counted, why per-backend blocks are closed and what a closed one must still say, why the project nav is built from what main answers rather than written down, and the search that force-opens everything defeating a laziness rule it had never heard of | #471, #472, #490 | Yes — one wiring for both scopes, the backends panel and the renderer rule |
...
**Script load order** — new renderer files must be added as `<script>` tags in `src/renderer/index.html` (current order ends ~lines 124–151). Add pure-logic modules **before** the files that consume them (e.g. before `sidebar.js`/`grid-view.js`/`app.js`).
