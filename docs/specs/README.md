...
| 30 | [Pi driven through its runtime, not through a terminal](30-pi-native.md) — **built in steps**; why the row stays Pi's and a marker in the transcript says how it was driven, the pipe wrapped as a PTY so stop/quit/re-key run unchanged, the neutral ops the backend decodes and the renderer draws, busy/idle through the same delivery a binding uses, and what was measured on the way | #568 | No — a backend, a core module, a migration, a renderer surface |
