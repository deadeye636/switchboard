...
- **Driven over RPC (`pi-native`, #568, spec 30).** The same store and the same transcript format; what differs
  is how the session is driven and one extra entry. A session `pi-native` drove carries a `custom` entry
  `switchboard-transport` with `{ transport: 'rpc' }` (`src/backends/pi/transport-marker.js`), appended once
  and held by Pi until the first message, so a session nobody wrote to leaves no marker. Pi's parser reads it
  into `row.transport` (parser version 7). What the app relies on from `pi --mode rpc`, measured on 0.84.4:
  `get_state` names the session within seconds of the start; `get_messages` is the history a view loads;
  `message_update` carries a delta per `contentIndex`, not a snapshot; `agent_start` / `agent_settled` are
  the busy and idle edges; a dialog an extension opens arrives as `extension_ui_request` and is answered
  with `extension_ui_response`; a failed model call is an assistant message with `stopReason: 'error'`,
  not an error response. Extension commands show in `get_commands`.
- **What Pi does not have**, measured on 0.84.4 and the reason several features are built as a per-spawn
  extension: no MCP client (the `mcpServers` strings in its bundle belong to a bundled Google SDK), no
...
| **Pi** | `model` (with model discovery), `provider`, `thinking`, `name`, `models`, `tools`, `excludeTools`, `noTools`, `noBuiltinTools`, `approval`, `offline`, `appendSystemPrompt`, `useTheme`, `noContextFiles`, and four applied at spawn rather than in the argv: `conventionPrompts` (#569), `subagentTool` + `subagentAgentsDir` (#634), `resourcesFrom` (#632) | **`--api-key`** — it would put a raw key on the COMMAND LINE, readable in any process listing. Pi reads its key from the environment; a template's `$VAR` env bundle (resolved at spawn, never on disk) is the only route we offer. Also `--mode json` and `--print` (non-interactive; `--mode rpc` is the transport of the separate `pi-native` backend, spec 30, not an option of this one), `--session-dir`/`--no-session` (they move or suppress the store we watch), arbitrary `--extension` paths (Switchboard passes only the extensions it generates per spawn: the live binding, the resources extension and, in `pi-native`, the runtime extension), and `--tui-mode` (#537 — `fullscreen` changes how the TUI drives the PTY Switchboard owns, and nobody has watched what that does to scrollback, resize and selection). Note `tools` **overrides Pi's own `defaultTools` setting**, which is what Pi falls back to when no `--tools` is given. |
