...
|---|---|---|
| **Claude** | `permissionMode`, `model`, `worktree` (+`worktreeName`), `chrome`, `addDirs`, `restricted`, `autocompact`, `mcpEmulation`, `afkTimeoutSec` | `--permission-prompts` / `--permission-prompt-tool` (print mode only — the flag #537 opened with), `--cloud` / `--environment` / `--teleport` (a session with no local transcript, so the scan cannot find, adopt or resume it), `--system-prompt-snapshot` (unmeasured). **`restricted` carries a warning that its label cannot**: it also ignores your settings files, which turns off Switchboard's attention hook, and it refuses `permissionMode: bypassPermissions` — that combination dies at spawn. |
