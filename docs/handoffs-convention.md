...
the prompt names. The project root itself is not a handoff directory either, and falls back the same way:
the save target is part of what the app will read packets from, so a `.` there would offer every `.md` in
