...
1. **Approach soundness** — is the chosen approach a correct way to satisfy the requirement? Flag any design flaw, wrong abstraction, or missed constraint. Check project conventions (CLAUDE.md): vanilla-JS renderer (no framework/bundler beyond the CodeMirror esbuild bundle), `execFile` over shell interpolation, append-only DB migrations, IPC = handler in main.js + binding in preload.js + renderer caller, English user-facing text, no PII in public artifacts.
